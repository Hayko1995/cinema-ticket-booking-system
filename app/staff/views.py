...
def index(request):
    ...
    b_data = (
        booking.objects.filter(show_date__gte=start_date)
        .values_list("show_date")
        .order_by("show_date")
        .annotate(total_seats=Sum("num_seats"))
    )
    r_data = (
        booking.objects.filter(show_date__gte=start_date)
        .values_list("show_date")
...
def movies(request):
    movies = (
        Film.objects.filter()
        .order_by("-id")
        .values_list("id", "movie_name", "date_added", "url", named=True)
    )
